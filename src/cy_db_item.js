...
let upsert = async function(item,opts){
    ...
    if (opts["$unset"]) {
      ...
      opts["$unset"].forEach((k, i) => {
        ...
        if (
          !item.sk.startsWith("fragment#index") &&
          !item.sk.startsWith("index#")
        ) {
          ops.push(
            docClient.send(
              new DeleteCommand({
                ...
                Key: {
                  pk: item.pk,
                  sk: item.sk.startsWith("fragment")
                    ? `fragment#index#${k}`
                    : `index#${k}`,
                },
              })
...
export class CyclicItemFragment {
  ...
  async indexes() {
    let indexes = await list_sks(
      `${this.parent.collection}#${this.parent.key}`,
      `fragment#index#`
    );
    return indexes.map((d) => {
      return d.split("#").slice(-1)[0];
