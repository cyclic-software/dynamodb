import { ValidationError } from "./cy_db_utils";

export const paths = function (item) {
  function iter(r, p) {
    var keys = Object.keys(r);
    // console.log(Array.isArray(r), r)
    if (Array.isArray(r)) {
      throw new ValidationError(
        `Array values are not supported in queries. Received: ${JSON.stringify(
          r
        )}`
      );
    }
...
export const path_get = function (o, p) {
  let prop = p.shift();
  if (prop) {
...
export const gen_expression = function (q_obj) {
  let p = paths(q_obj);
  let expression = [];
  let attr_vals = {};
