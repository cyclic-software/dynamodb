...
import { gen_expression } from "./expressions";

export class CyclicCollection {
  /*
  ...
  async filter(q = {}, segments = 3, next = null) {
    ...
    let segment_results = await Promise.all(
      scans.map((s) => {
...
export default CyclicCollection;
